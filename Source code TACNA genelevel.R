...
combat_consensus_ica_degr_clustering = function(expression_data, #input expression data probesets/genes in rows and samples in columns
                                           ...
                                           data_with_chr_bp_mapping, #Genomic mapping file with the first column as "ENTREZID". Other columns are 'GENETITLE', 'SYMBOL','CHR_Mapping','BP_Mapping'
...
permutation_testing_to_find_degr = function(dataset #Consensus estimated sources matrix
  ...
  if(row_num!=dim(data_with_chr_bp_mapping)[1])
  {
      ...
      if(length(which_probe_extreme_valued)>0){
        ...
        for(num in 1:length(extreme_valued_runs)){
          ...
          write.table(extreme_valued_section,
                      file = paste(file_genelevel_bp,paste(Title,"_CHR", extreme_valued_section$CHR_Mapping[1],"_BPSTART",round(extreme_valued_section$BP_Mapping[1]), "_BPEND",
          ...
          write.table(extreme_valued_section_sort_by_value,
                      file = paste(file_genelevel_value,paste(Title,"_CHR", extreme_valued_section_sort_by_value$CHR_Mapping[1],"_BPSTART",round(extreme_valued_section_sort_by_value$BP_Mapping[1]), "_BPEND",
    ...
    if(TACNA_profiling){
      ...
      {
        row_num_match[alteration_probes] = which(rownames(full_dataset)==extreme_valued_section_summary$ENTREZID[alteration_probes])
      ...
      matching_vec = match(data_with_chr_bp_mapping$ENTREZID,rownames(full_dataset))
